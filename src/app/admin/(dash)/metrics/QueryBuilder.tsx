...
export function QueryBuilder({ datasets, chartTypes }: { datasets: Dataset[]; chartTypes: { key: string; label: string }[] }) {
  const [openBuilder, setOpenBuilder] = useState(false);
  if (!openBuilder) {
    return (
      <button className="btn-secondary" onClick={() => setOpenBuilder(true)}>
        <Plus size={16} /> New chart
      </button>
    );
  }
  return <Builder datasets={datasets} chartTypes={chartTypes} onClose={() => setOpenBuilder(false)} />;
}

function Builder({ datasets, chartTypes, onClose }: { datasets: Dataset[]; chartTypes: { key: string; label: string }[]; onClose: () => void }) {
  ...
  // Re-run the preview whenever the definition changes.
  useEffect(() => {
    start(async () => {
  ...
  function onDataset(key: string) {
    setDataset(key);
    const first = datasets.find((d) => d.key === key)?.dimensions[0].key ?? "";
    setDimension(first);
  }
  ...
  function save() {
    setSaving(true);
    setError(null);
    saveChart({ name: name.trim() || suggestName(), dataset, dimension, chartType, from: from || undefined, to: to || undefined }).then((res) => {
      setSaving(false);
  ...
  return (
    <div className="card p-5">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-sm font-semibold text-ink">New chart</h2>
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        <button className="btn-primary" onClick={save} disabled={saving}>
          {saving ? <Loader2 className="animate-spin" size={16} /> : <Save size={16} />} Save chart
        </button>
      </div>
    </div>
  );
